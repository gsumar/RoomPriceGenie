...
| File | Purpose |
|------|---------|
| `qa/qa_pure_python.py` | Reimplements the full pipeline logic in **pure Python** (no pandas, no external dependencies) and generates an independent KPI CSV |
| `qa/qa_compare.py` | Loads both the pipeline output and the QA output and compares them row by row |
| `qa/kpi_1035_qa_pure_python.csv` | The reference output generated by the QA script |

### Why Pure Python?

The QA script deliberately avoids pandas and any shared library with the pipeline. This means:
- If both implementations agree → the logic is correct
- If they disagree → there is a real data quality or business logic issue, not just a library quirk
...
**Step 2 — Compare against the pipeline output:**
```bash
...
**If there are mismatches**, the comparison script will show exactly which rows differ and by how much, making it easy to trace back to the source of the issue.

### What Is Validated

- ✅ Row count (31 rows for May 2026)
- ✅ `OCCUPANCY_PERCENTAGE` per night (within configurable float tolerance)
- ✅ `TOTAL_NET_REVENUE` per night
- ✅ `ADR` per night
- ✅ Sort order (`NIGHT_OF_STAY` descending)
- ✅ Zero-fill for dates with no activity
